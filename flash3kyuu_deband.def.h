...
public:
    ...
    flash3kyuu_deband_parameter_storage_t( int range, unsigned short Y, unsigned short Cb, unsigned short Cr, int ditherY, int ditherC, int sample_mode, int seed, bool blur_first, bool diff_seed_for_each_frame, int opt, bool mt, PRECISION_MODE precision_mode, bool keep_tv_range, INPUT_MODE input_mode, int input_depth, bool enable_fast_skip_plane, RANDOM_ALGORITHM random_algo_ref, RANDOM_ALGORITHM random_algo_dither )
    {
        _range = range; 
        _Y = Y; 
        _Cb = Cb; 
...
#define FLASH3KYUU_DEBAND_CREATE_CLASS(klass) new klass( child, flash3kyuu_deband_parameter_storage_t( range, (unsigned short)Y, (unsigned short)Cb, (unsigned short)Cr, ditherY, ditherC, sample_mode, seed, blur_first, diff_seed_for_each_frame, opt, mt, (PRECISION_MODE)precision_mode, keep_tv_range, (INPUT_MODE)input_mode, input_depth, enable_fast_skip_plane, (RANDOM_ALGORITHM)random_algo_ref, (RANDOM_ALGORITHM)random_algo_dither ) )
