...
protected:
    ...
    double _random_param_ref; 
    double _random_param_grain; 
...
public:
    ...
    flash3kyuu_deband_parameter_storage_t( int range, unsigned short Y, unsigned short Cb, unsigned short Cr, int grainY, int grainC, int sample_mode, int seed, bool blur_first, bool dynamic_grain, int opt, bool mt, DITHER_ALGORITHM dither_algo, bool keep_tv_range, PIXEL_MODE input_mode, int input_depth, PIXEL_MODE output_mode, int output_depth, RANDOM_ALGORITHM random_algo_ref, RANDOM_ALGORITHM random_algo_grain, double random_param_ref, double random_param_grain )
    {
        ...
        _random_param_ref = random_param_ref; 
        _random_param_grain = random_param_grain; 
    }
};

typedef struct _FLASH3KYUU_DEBAND_RAW_ARGS
{
    AVSValue child, range, Y, Cb, Cr, grainY, grainC, sample_mode, seed, blur_first, dynamic_grain, opt, mt, dither_algo, keep_tv_range, input_mode, input_depth, output_mode, output_depth, random_algo_ref, random_algo_grain, random_param_ref, random_param_grain;
