...
int params_set_by_string(f3kdb_params_t* params, const char* name, const char* value_string)
{
    ...
    if (!_stricmp(name, "input_mode")) { return params_set_value_by_string(&params->input_mode, value_string); }
    if (!_stricmp(name, "input_depth")) { return params_set_value_by_string(&params->input_depth, value_string); }
